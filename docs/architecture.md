...
- Classes / Attendance / QR check-in (`classes`, `class_sessions`, `attendance`): scoping club via `is_club_member` en rejoignant `classes`/`class_sessions` (même pattern que `group_members`, pas de dénormalisation de `club_id`). Check-in par code court: `rotate_checkin_code()` (coach only, `security definer`) génère un token valide 20 min sur `class_sessions.checkin_code`/`checkin_code_expires_at`, une nouvelle génération invalide toujours la précédente. `get_checkin_session_info()` (`security definer`, `grant ... to anon, authenticated`) expose en lecture publique le strict minimum (nom du cours/club, horaire) uniquement pendant la fenêtre de validité. `checkin_to_class_session()` (`security definer`, authenticated only) marque l'appelant présent après re-vérification du code et de son appartenance au club — jamais d'écriture `attendance` en libre RLS pour l'auto check-in. Voir `00000000000008_classes_attendance.sql`.
...
- Messagerie de club, Coach Dashboard, Compétition (gameplan/post-fight review), Computer Vision (Video, VideoAnnotation), paiements: scope complet documenté (pas construit) dans `docs/decisions/0007-v3-scope-roadmap.md`, inchangé par `docs/decisions/0008`. Club/ClubMember/Groups livrés en P3.1 (voir ci-dessus et migration `00000000000007_club_groups.sql`); Athlete/Match/Sequence livrés en P2 (`docs/decisions/0009`); Classes/Attendance/QR check-in livrés en P3.2 (voir ci-dessus et migration `00000000000008_classes_attendance.sql`).
